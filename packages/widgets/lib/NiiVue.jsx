  ...
  useEffect(() => {
    async function runAsyncStartupFuncs() {
      ...
      if (niivuejs.webGL2Supported()) {
        ...
        let backColor = [0, 0, 0, 1];
        let show3Dcrosshair = false;
        let clipPlaneColor = [0.7, 0, 0.7, 0.5];
        for (const arg in cmdLineArgs) {
          switch (arg) {
            ...
            case "backColor":
              {
                backColor = cmdLineArgs[arg];
              }
              break;
            case "clipPlaneColor":
                {
                    clipPlaneColor = cmdLineArgs[arg];
                }
                break;
            case "show3Dcrosshair":
              {
                show3Dcrosshair = cmdLineArgs[arg];
              }
              break;
          }
        }
